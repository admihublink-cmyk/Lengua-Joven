...
test.describe('Panel — navegación post-login', () => {
  ...
  test('sección Usuarios accesible', async ({ page }) => {
    await loginAdmin(page)
    const navUsuarios = page.getByRole('link', { name: /Usuarios/i }).or(page.getByText(/Usuarios/i)).first()
  ...
  test('sección Planteles accesible', async ({ page }) => {
    await loginAdmin(page)
    const navPlanteles = page.getByRole('link', { name: /Planteles/i }).or(page.getByText(/Planteles/i)).first()
    await navPlanteles.click()
  ...
  test('sección Legal y ARCO carga sin error', async ({ page }) => {
    await loginAdmin(page)
    const navLegal = page.getByRole('link', { name: /Legal/i }).or(page.getByText(/Legal/i)).first()
    await navLegal.click()
