...
test.describe('Landing page', () => {
  ...
  test('navbar visible con botón de login', async ({ page }) => {
    ...
    await expect(page.getByAltText('INJUVE NL').first()).toBeVisible()
  })
  ...
  test('links de navegación funcionan (scroll)', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: '¿Quiénes somos?' })).toBeInViewport()
  })

  test('sección oferta educativa muestra idiomas', async ({ page }) => {
    await page.goto('/')
    await page.getByRole('link', { name: /Oferta educativa/i }).click()
    await expect(page.getByRole('heading', { name: 'Oferta educativa' })).toBeInViewport()
  })
})
...
test.describe('Pre-registro', () => {
  test('se abre el formulario', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Pre-registro' })).toBeVisible()
    await expect(page.getByPlaceholder('Ej. Ana González Martínez')).toBeVisible()
  })
  ...
  test('valida CURP inválido', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Enviar pre-registro' }).click()
    await expect(page.getByText('El CURP es requerido (18 caracteres).')).toBeVisible()
  })
})
